...
function showBanner(n){
    for(let banner of banners){banner.classList.remove('active')}

    for(let counter of counters){counter.classList.remove('active')}
...
function nextBanner(){
    if(currentBanner+1 >= banners.length){currentBanner = 0}
    else{currentBanner++}
