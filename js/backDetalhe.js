...
//cria os vetores para sugestão de produtos
var idProds=[];
var descProds=[];
var precoProds=[];
var tamanhoProds=[];
...
window.onload=()=>{pesquisaProduto();}

//funcão para pesquisar prod pelo id
function pesquisaProduto(){
    limpa_vetores();
    //requisita o backend pelo id
    fetch('http://localhost/devstore--exercicio-main/'+'produtos.php?idProd='+id)
    .then(response=>response.json()).then(data=>{ //manipula os dados da resposta
        ...
        for(var j=0; j<1;j++){
            ...
            var categoria = data[j].caegoriaProd;
